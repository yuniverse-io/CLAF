...
from src.tokenizer import build_tokenizer_and_embeddings
...
# 2. 토크나이징
review_train, review_val, review_test, \
embedding_matrix, total_words = \
    build_tokenizer_and_embeddings(
...
# 3. 모델
model = build_model(total_words, embedding_matrix, config)
model.summary()

# 4. 학습
print("모델 학습 시작...")
model.fit(
    review_train, train_label,
    validation_data=(review_val, val_label),
...
# 5. 평가
predicted = model.predict(review_test)
