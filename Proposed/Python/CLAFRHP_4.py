...
class MFB(layers.Layer):
    ...
    두 모달리티 벡터를 bilinear pooling으로 융합 (concat 대체)
    
    output_dim, factor_dim을 입력 차원 기반 자동 계산:
      output_dim = input_dim × output_ratio  (default 2.0 → 768 입력시 1536)
      factor_dim = input_dim × factor_ratio  (default 0.667 → 768 입력시 ≈512)
    
    수식:
        z = SumPool(U·v_t ⊙ V·v_i, k)   ← element-wise multiply 후 k개 factor 합산
        z = sign(z) * sqrt(|z| + eps)    ← power normalization
        z = L2_normalize(z)              ← L2 normalization
        out = output_proj(z)             ← 최종 projection
    
    입력: inputs = [text_vec, image_vec], 각 (B, D)
    출력: fused (B, output_dim)
    """
    def __init__(self, output_ratio=2.0, factor_ratio=0.667, k=5, dropout=0.1, **kwargs):
        super().__init__(**kwargs)
        self.output_ratio = output_ratio
        self.factor_ratio = factor_ratio
        self.k = k
        self.dropout_rate = dropout
        # build()에서 입력 차원 받아서 자동 계산
        self.output_dim = None
        self.factor_dim = None

    def build(self, input_shape):
        # input_shape는 [text_shape, image_shape] 형태 (list of TensorShape)
        # 두 입력 모두 (B, D)이므로 D를 가져옴
        ...
        if input_dim is None:
            raise ValueError("MFB: 입력 차원이 None입니다.")
        
        # 자동 계산
        self.output_dim = int(input_dim * self.output_ratio)
        self.factor_dim = int(input_dim * self.factor_ratio)
        
        # Layer 생성 (build에서 동적 차원 사용)
        self.U = layers.Dense(self.factor_dim * self.k)   # text projection
        self.V = layers.Dense(self.factor_dim * self.k)   # image projection
        self.dropout = layers.Dropout(self.dropout_rate)
        self.output_proj = layers.Dense(self.output_dim)
        
        print(f"[MFB] input_dim={input_dim}, output_dim={self.output_dim}, "
              f"factor_dim={self.factor_dim}, k={self.k}")
        super().build(input_shape)
    
    def call(self, inputs, training=False):
        # inputs: [text_vec, image_vec], 각 (B, D)
        text_vec, image_vec = inputs
        
        # 1) Project to factor space
        u = self.U(text_vec)    # (B, factor_dim * k)
        v = self.V(image_vec)   # (B, factor_dim * k)
        
        # 2) Element-wise multiply (bilinear interaction)
        z = u * v               # (B, factor_dim * k)
        
        # 3) Dropout
        z = self.dropout(z, training=training)
        
        # 4) Sum pool over k factors
        # (B, factor_dim * k) → (B, factor_dim, k) → sum over k → (B, factor_dim)
        B = tf.shape(z)[0]
        z = tf.reshape(z, [B, self.factor_dim, self.k])
        z = tf.reduce_sum(z, axis=-1)   # (B, factor_dim)
        
        # 5) Power normalization
        z = tf.sign(z) * tf.sqrt(tf.abs(z) + 1e-8)
        
        # 6) L2 normalize
        z = tf.nn.l2_normalize(z, axis=-1)
        
        # 7) Output projection
        return self.output_proj(z)   # (B, output_dim)
...
class BidirectionalCrossAttentionFusion(layers.Layer):
    ...
    def __init__(
        ...
        d_ff=3072,
        dropout=0.1,
        mfb_output_ratio=2.0,
        mfb_factor_ratio=0.667,
        mfb_k=5,
        **kwargs,
    ):
        super().__init__(**kwargs)
        ...
        # 추가된 MFB 층
        self.mfb = MFB(
            output_ratio=mfb_output_ratio,
            factor_ratio=mfb_factor_ratio,
            k=mfb_k,
            dropout=dropout,
        )
    ...
    def call(self, text_repr, image_repr, text_mask, training=False):
        ...
        # 3) AttentionPool (시퀀스 압축, 모든 cross-attended 토큰 활용)
        t2i_pooled = self.text_pool(t2i, mask=text_mask)   # (B, 768)
        i2t_pooled = self.image_pool(i2t, mask=None)        # (B, 768)

        # 4) MFB Fusion (concat 대체, 두 모달리티 bilinear interaction)
        # 표준 Keras 패턴: 두 입력을 list로 묶어서 전달
        fused = self.mfb([t2i_pooled, i2t_pooled], training=training)   # (B, 1536)
